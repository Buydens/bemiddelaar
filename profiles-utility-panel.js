...
const filterResultBadge = document.querySelector("#filterResultBadge");
...
function renderFilterSummary(resultCount) {
  ...
  filterResultBadge.textContent = String(resultCount);
  filterResultPreview.textContent = `${resultCount} ${resultCount === 1 ? "profiel" : "profielen"}`;
  filterButton.classList.toggle("is-active", count > 0 || !filterPanel.hidden);
}
...
function renderDependentContent() {
  ...
  if (filterPanel.hidden && !overviewFocused) openDependentPanels();
  else dependentPanels.forEach((panel) => setPanelCollapsed(panel, true, false));
  updateWorkspaceStatus();
}
...
function updateWorkspaceStatus() {
  ...
  if (!filterPanel.hidden) {
    ...
    visiblePanelLabel.textContent = "Overzicht + hulppaneel · profielcontext als rails";
  } else if (!count) {
...
function clampFilterWidth(width) {
  const reservedWidth = 500;
  const availableMax = Math.max(280, workspace.clientWidth - reservedWidth);
  return Math.min(Math.max(width, 280), Math.min(520, availableMax));
}
...
document.querySelectorAll(".collapse-dependent").forEach((button) => {
  button.addEventListener("click", () => {
    setPanelCollapsed(button.closest(".dependent-panel"), true, false);
    updateWorkspaceStatus();
  });
});
...
utilityDivider.addEventListener("keydown", (event) => {
  ...
  setFilterWidth(currentWidth + (event.key === "ArrowLeft" ? 20 : -20));
});
